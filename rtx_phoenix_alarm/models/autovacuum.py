...
class RtxAlarmAutovacuum(models.TransientModel):
    _name = "rtx.alarm.autovacuum"
    ...
    def autovacuum(self, days):
        ...
        records = self.env["rtx.phoenix.alarm.pool"].search(
            [("create_date", "<=", fields.Datetime.to_string(deadline))]
        )
        nb_records = len(records)
        records.unlink()    # 删除告警运行日志表
        _logger.info("告警日志 - %s '%s' 告警日志已删除", nb_records, "rtx.phoenix.alarm.pool")
        return True
