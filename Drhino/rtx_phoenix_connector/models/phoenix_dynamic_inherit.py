...
class RtxPhoenixDynamicMeasurements(models.Model):
    _inherit = 'rtx.phoenix.dynamic.measurements'

    def check_monitor_data(self):
        """去查阈值报警功能的阈值设定,每个测点可以设置2个报警级别。计算数据结果写入到异常清单池中。"""
        alarm_obj = self.env['rtx.phoenix.alarm.pool']
        ...
        if len(monitor_records):
            for dynamic_id in monitor_records:
                serial_id = equipment_obj.search([('serial_no', '=', dynamic_id.PointID)])  # 查询设备/测点内容
                pv = {
                ...
                # 检查是否为 None
                if dynamic_id.high_alarm is not None and dynamic_id.lower_warning is not None:
    ...
    def check_temperature_monitor_data(self):
        ...
        alarm_obj = self.env['rtx.phoenix.alarm.pool']
        equipment_obj = self.env['maintenance.equipment']
        configurator_obj = self.env['rtx.phoenix.configurator']
        monitor_obj = self.env['rtx.phoenix.dynamic.measurements']
        ...
        if len(unique_ids):
            for dynamic_id in unique_ids[0]:
                ...
                if not configurator_interval.value:
                    ...
                    # # 检查是否为 None
                    if monitor_value is not None and configurator_coefficient.value is not None:
                        # 告警动作逻辑，从最高告警信息反向查找(报警 > 警告)
