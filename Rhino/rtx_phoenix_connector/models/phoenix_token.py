...
class RtxPhoenixToken(models.Model):
    _name = 'rtx.phoenix.token'
    ...
    url = fields.Char(string='接口地址', default='', help='必填。接口地址')
    ...
    def action_token_from_phoenix(self):
        ...
        # api config
        phoenix = self.env['rtx.phoenix.token'].search([('name', '=', '获取PhoenixAPI网关接口调用token')])
