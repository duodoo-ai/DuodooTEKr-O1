...
class RtxSocketServerStart(models.Model):
    _name = 'rtx.socket.server.start'
    ...
    def action_clear_alarm(self):
        ...
        """
        alarm_obj = self.with_user(1).env['rtx.phoenix.alarm.pool']
    ...
    def send_value_to_socket(self):
        # 设备的 IP 地址和端口号
        device_obj = self.env['rtx.phoenix.audible.address']
    ...
    def open_alarm_value_to_socket(self):
        ...
        port = int(device_record.port)
        # 查找：	打开声音和灯光 指令动作
        action_record = self.env['rtx.socket.server.start'].search([('action', '=', '打开声音和灯光')]).name
    ...
    def close_alarm_value_to_socket(self):
        ...
        port = int(device_record.port)
        # 查找：	打开声音和灯光 指令动作
        action_record = self.env['rtx.socket.server.start'].search([('action', '=', '清除告警')]).name
