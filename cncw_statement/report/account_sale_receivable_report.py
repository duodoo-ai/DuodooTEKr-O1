...
class AccountSaleReceivableReport(models.Model):
    ...
    remaining_amount = fields.Float('发货欠款金额', digits='Product Price', group_operator="sum")
    partner_id = fields.Many2one('res.partner', '发货时间')
    partner_id = fields.Many2one('res.partner', '发货金额')
    date_invoice = fields.Date('开票时间')
    amount_total = fields.Float('开票金额', digits='Product Price', group_operator="sum")
    payment_date = fields.Many2one('res.partner', '来款时间')
    payment_amount = fields.Float('来款金额', digits='Product Price', group_operator="sum")
    partner_id = fields.Many2one('res.partner', '合同金额')
    partner_id = fields.Many2one('res.partner', '合同欠款金额')
    partner_name = fields.Char('客户名称')
    overdue_amount = fields.Float('超期应收款', digits='Product Price', group_operator="sum")
    ...
    invalid_amount = fields.Float('作废金额', digits='Product Price', group_operator="sum")
    exchange_rate = fields.Float('汇率', digits='Exchange Rate', group_operator="avg")
    payment_rate = fields.Float('收款比率', digits='Exchange Rate', group_operator="avg")
    no_overdue_amount = fields.Float('未超期应收款', digits='Product Price', group_operator="sum")
