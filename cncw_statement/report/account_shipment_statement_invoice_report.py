...
class account_shipment_statement_invoice(models.Model):
    ...
    product_qty = fields.Float('出货数量', digits='Product Unit of Measure', group_operator="sum")
    net_weight = fields.Float('出货重量', digits='Stock Weight', group_operator="sum")
    product_uom = fields.Many2one('uom.uom', '单位', required=False, )

    currency_id = fields.Many2one('res.currency', '币别' )
    price_unit = fields.Float(string='单价', digits='Product Price', group_operator="avg")
    delivery_amount = fields.Float('出货金额', digits='Product Price', group_operator="sum")
    checked_amount = fields.Float('对账金额',  digits='Product Price', group_operator="sum")
    local_checked_amount = fields.Float('本币对帐金额',  digits='Product Price', group_operator="sum")
    adjust_amount = fields.Float('对账调整金额',  digits='Product Price', group_operator="sum")
    unchecked_amount = fields.Float('未对账金额',  digits='Product Price', group_operator="sum")
    invoiced_amount = fields.Float('已开票金额', digits='Product Price', group_operator="sum")
